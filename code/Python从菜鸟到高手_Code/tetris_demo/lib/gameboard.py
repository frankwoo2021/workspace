...
class GameBoard():
    ...
    def ableMove(self, coord, direction=None):
        ...
        for x, y in self.currentGame.getAbsoluteCoords(direction, coord[0], coord[1]):
    ...
    def moveDown(self):
        ...
        if self.ableMove([self.current_coord[0], self.current_coord[1] + 1]):
            ...
            x_min, x_max, y_min, y_max = self.currentGame.getRelativeBoundary(self.current_direction)
    ...
    def mergeTetris(self):
        for x, y in self.currentGame.getAbsoluteCoords(self.current_direction, self.current_coord[0], self.current_coord[1]):
            self.board_data[x + y * self.width] = self.currentGame.shape
        self.current_coord = [-1, -1]
        self.current_direction = 0
        self.currentGame = tetrisShape()
    '''移出整行都有小方块的'''
    ...
    def createNewTetris(self):
        ...
        if self.ableMove([self.init_x, -y_min]):
            ...
            self.currentGame = self.next_tetris
            self.next_tetris = self.getNextTetris()
        else:
            self.is_gameover = True
        self.shape_statistics[self.currentGame.shape] += 1
    '''获得下个俄罗斯方块'''
    def getNextTetris(self):
        '''random.randint(1, 7)'''
      #  return tetrisShape(random.randint(1, 7))
        return tetrisShape(6)
    '''获得板块数据'''
    ...
    def getCurrentTetrisCoords(self):
        return self.currentGame.getAbsoluteCoords(self.current_direction, self.current_coord[0], self.current_coord[1])
    '''重置'''
    def reset(self):
        # 记录板块数据
...
class ExternalBoard(QFrame):
    ...
    def __init__(self, parent, blockSize, inner_board):
        ...
        self.setFixedSize(blockSize * inner_board.width, blockSize * inner_board.height)
    ...
    def paintEvent(self, event):
        ...
        for x in range(self.inner_board.width):
            for y in range(self.inner_board.height):
                ...
                drawCell(painter, x * self.blockSize, y * self.blockSize, shape, self.blockSize)
        # 绘制正在下落的方块
        for x, y in self.inner_board.getCurrentTetrisCoords():
            shape = self.inner_board.currentGame.shape
            drawCell(painter, x * self.blockSize, y * self.blockSize, shape, self.blockSize)
